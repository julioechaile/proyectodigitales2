...
//variables globales par usar
int buttonState = 0;         
int count_value =0;
int prestate =0; //flag del task
...
static void mcpwm_example_brushed_motor_control(void *arg)
{
    ...
    //seteo del pin a usar con el boton
...
while(1){
  ...
  //el duty sube mientras se presiona el boton
  if (buttonState == 1 && prestate == 0) {
    count_value = count_value + 10;
    float duty_cycle = count_value; //aca se crea el float duty_cycle
    //duty_cycle sube de a 10 con cada presionar del boton
...
void app_main(void)    //solo crea el task
{
